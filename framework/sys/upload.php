...
class Upload{
	...
	public static function image($name, $path) {
		if (self::isImage($_FILES[$name]['tmp_name'])) {
			return self::uploadFile($name, $path);
		}
		return array('error' => '2', 'msg' => '错误的文件类型');
	}
 ...
 * class:上传控件的class样式补充(string :btn btn-primary;)<br/>
...
<input name="YYUC_UPLOAD_file" style="position: absolute;height: 100%;width:{$width}px;top: 0;right: 0;z-index:2" type="file"/>
<input src="{$backgroundImg}" type="{$btntype}" style="display:block; position: absolute;height: 100%;width:100%;z-index:1;line-height: {$lineheight}px;font-size:{$fontSize}px;color:{$fontColor};text-align: center;" class="{$class}" value="{$text}"/>
</span>
...
static function save_upload_file($subpath, $funupload=null,$funcut=null){
	//临时目录清空
    File::remove_dir(APP_PUBLIC_PATH . 'upload/temp/', (time() - 86400));
    File::creat_dir_with_filepath(APP_PUBLIC_PATH . $subpath);
	if(!empty($_FILES)&&isset($_FILES['YYUC_UPLOAD_file'])){
		...
		if($f['size']>$thissize){
            ...
            if (!move_uploaded_file($f['tmp_name'], APP_PUBLIC_PATH . $filename)) {
				Response::write('<script>window.parent.'.$thisid.'=1;window.parent.'.$thisid.'_failure("server error",4);</script>');
			}else{
				if($funupload !==null){
                    call_user_func_array($funupload, array(APP_PUBLIC_PATH . $filename));
				}
        ...
        if (!file_exists(APP_PUBLIC_PATH . $filename)) {
			return false;
		}
		//剪切之后的处理
        $img_r = imagecreatefromstring(file_get_contents(APP_PUBLIC_PATH . $filename));
		if($img_r===false){
			Response::write('no');
		}
		$dsw = Request::post('w');
        ...
        imagejpeg($dst_r, APP_PUBLIC_PATH . $filename2, 90);
		if($filename2!=$filename){
            unlink(APP_PUBLIC_PATH . $filename);
		}
		if($funcut !==null){
            call_user_func_array($funcut, array(APP_PUBLIC_PATH . $filename2));
		}
    ...
    public static function uploadFile($name, $subpath) {
        // 临时目录清空
        File::remove_dir(APP_PUBLIC_PATH . 'upload/temp/', (time() - 86400));
        File::creat_dir_with_filepath(APP_PUBLIC_PATH . $subpath);
        ...
        if (!empty($_FILES) && isset($_FILES[$name])) {
            ...
            if ($file['size'] > (1024000 * config('maxUploadSize'))) {
                return array('error' => 1, 'msg' => '上传的文件不能超过' . config('maxUploadSize') . 'M');
            }

            if (stripos($filename, '.php') !== false) {
                return array('error' => 2, 'msg' => '错误的文件类型');
            }
            if (!move_uploaded_file($file['tmp_name'], APP_PUBLIC_PATH . $filename)) {
                return array('error' => '3', 'msg' => '文件上传失败');
            }
            return array('error' => 0, 'msg' => '上传成功', 'data' => array('name' => $file['name'], 'path' => $filename));
        } else {
            return array('error' => 4, 'msg' => '上传失败');
        }
    }
...
private static function _resave_upload_file($key,$folderpath=null,$isFile = false){
	if(trim($key)==''){
		...
		//删除上次的文件地址
        @unlink(APP_PUBLIC_PATH . substr($key, 4));
		return false;
	}elseif (strpos($key, '@-@')!==false){
		//修改后删除上次的文件地址
		$keys = explode('@-@', $key);
		$key = $keys[0];
        @unlink(APP_PUBLIC_PATH . $keys[1]);
	}
	if($folderpath===null){
		$folderpath = 'upload/auto/'.date('Y/m/d',time());
	}
    if (!$isFile) {
        File::creat_dir(APP_PUBLIC_PATH . $folderpath);
        $tsubppath = '/' . $folderpath . '/' . basename($key);
        $newpath = APP_PUBLIC_PATH . $folderpath . '/' . basename($key);
    } else {
        File::creat_dir_with_filepath(APP_PUBLIC_PATH . $folderpath);
        $tsubppath = '/' . $folderpath;
        $newpath = APP_PUBLIC_PATH . $folderpath;
    }
	@unlink($newpath);
    rename(APP_PUBLIC_PATH . $key, $newpath);
