...
class StringUtil {
     ...
     * @return bool
     */
    public static function startWith($haystack, $needle, $case = true) {
        if ($case) {
     ...
     * @return bool
     */
    public static function endWith($haystack, $needle, $case = true) {
     ...
     * @return string
     */
    public static function findFirstStringByReg($str, $reg) {
        $out = array();
     ...
     * @return string
     */
    public static function str2binary($str) {
     ...
     * @return bool|string
     */
    public static function binary2str($str) {
        $len = strlen($str);
        $data = '';
        for ($i = 0; $i < ($len / 8); $i++) {
    ...
    public static function encrypt($string, $action = 'ENCODE', $hash = '') {
    ...
    public static function encryption($str, $key = '') {
        //加密的算法是取余
        return self::encrypt($str, 'E', $key . config('encrypt_key'));
    }
    ...
    public static function decryption($str, $key = '') {
        return self::encrypt($str, 'D', $key . config('encrypt_key'));
    }
